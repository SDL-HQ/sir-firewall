...
2. **Registry-managed benchmark suites** — `tests/domain_packs/*.csv` and `tests/scenario_packs/*.json`. These are prompt sets run through the gate to produce evidence and are listed in `spec/packs/pack_registry.v1.json`. They are discoverable through `sir packs list` when their visibility is `public`.
