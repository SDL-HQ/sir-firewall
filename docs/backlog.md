...
| Proof and archive coverage tables can drift from the generated report | Planned | `docs/latest-run.html`, `proofs/template.html`, and `proofs/runs/index.html` hand-maintain duplicate full-gate coverage lookup tables. A test can detect divergence, but the durable fix is to emit these tables from `tools/rule_coverage_report.py` output or have the pages read `rule-coverage.json` directly. |
