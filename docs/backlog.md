...
These substantive prompt sets are active registry entries. Public entries are discoverable through `sir packs list`; `mental_health_clinical` has `encoded` visibility. Only `generic_safety`, `support_operator_override`, `data_exfiltration_pressure`, and `eu_ai_act_compliance_pressure` currently have same-named ISC policy packs and evaluate through the `--pack` route. The other four active suites listed below currently produce systemic-reset blocks instead of meaningful suite evaluation.
...
| Item | Status | Notes |
|---|---|---|
| `sir packs list` presents registry discoverability as execution availability | Planned | `_cmd_packs_list` filters registry entries on public visibility and does not check whether a same-named ISC policy pack exists. `account_recovery_fraud`, `scenario_injection_chain` and `scenario_tool_injection` are therefore listed as active despite producing systemic-reset blocks when selected through `--pack`. `mental_health_clinical` has the same execution constraint but is absent from the listing because its visibility is `encoded`. Consider marking entries without a corresponding ISC policy pack, or adding a column distinguishing registry status from execution availability. |
| Raw structured and tool-result JSON parsed before size validation | Planned | Both string ingress modes call `json.loads()` on the complete raw document before any field or content-length validation. The 4000-character content limit therefore does not bound parser work. A raw envelope size cap cannot be derived exactly from the content limit because JSON structural and escaping overhead is unbounded, so it would require a documented configuration value. `RecursionError` and `MemoryError` are caught for structured requests, so those in-process failures BLOCK rather than raising, but parser cost is still attacker-proportional and a process-level OOM kill cannot be caught in-process. |
| Non-string ISC payloads coerced with `str()` before size evaluation | Investigating | The payload size fast-path applies only when `isc.payload` is already a string. Non-string values retain the existing `str()` coercion, so a caller supplying an object with an expensive or adversarial `__str__` can still force work before rejection. Requiring `isc.payload` to be a string would close this but would change behaviour for currently accepted non-string inputs, so it is not folded into the size fix. |
...
| Homoglyph substitution can bypass multi-token rules | Addressed in 2.3.0 | `normalize_obfuscation()` now applies the unchanged nine-entry `_HOMOGLYPH_ASCII_MAP` to evaluated text on every return path. Regression tests record the former `PASS` behavior for mapped characters outside recovered markers and the current `BLOCKED` result. Structural override exposure intentionally continues to inspect the raw payload and remains homoglyph-blind. |
| Recorded normalisation limitations | Characterised | Tests now record that marker recovery is not idempotent (`"obey"` yields `"obey obey"`, then `"obey obey obey"`) and that an invisible character inside a marker token can introduce a token separator. The behaviors remain unchanged and outside the claimed normalization invariants. |
| Homoglyph obfuscation has zero registry-suite coverage | Planned | No row in any registered CSV or scenario suite contains a character from `_HOMOGLYPH_ASCII_MAP`. Adding obfuscation variants would change suite hashes and coverage denominators, so it requires a separate benchmark-content decision and evidence rerun rather than being folded into a runtime patch. |
...
| Published coverage tables can drift from the generated report | Implemented | `tools/rule_coverage_report.py` owns delimited regions in the current public HTML surfaces, including `docs/latest-run.html`, `docs/domain-packs.html`, the current audit pages, the run index, and `proofs/template.html`; a drift test compares each region with fresh generator output. Historical files under `proofs/` remain immutable evidence artefacts and are checked, not rewritten. |
...
| Bind provider-supplied call identifiers into signed certificates | Planned | No provider-supplied request ID, response ID, or equivalent is currently captured anywhere: neither the signed certificate payload nor the optional downstream sidecar contains one. Implementation requires: (1) extract a defined provider-supplied ID from the LiteLLM/provider response object; (2) decide cardinality and representation, given one identifier per successful call rather than per run; (3) persist it in structured run evidence rather than only a sidecar; (4) add either the individual IDs or a deterministic digest over an ordered ID record to the certificate dictionary before payload serialization; (5) update the evidence contract, verifier expectations, archive and export tooling, and tests; and (6) regenerate evidence under the revised proof format. |
| Signed certificate omits runner image identity | Planned | Certificates record `commit_sha` and `ci_run_url`, but not the runner image identifier or OS version, so the build environment that produced a signed result is not recoverable from the certificate itself. Capturing the runner image identifier in the signed payload would close this gap. This sits alongside the existing `provider_response_id` and content-evaluation assertion rows. |
| Contract validator does not resolve signing key identifiers | Investigating | `validate_certificate_contract.py` accepts a `signing_key_id` that does not resolve to an entry in the key registry because it checks only that the field is a non-empty string and that the key schema declares verifier expectations. Decide whether registry resolution belongs in contract validation or is correctly left to `verify_certificate.py`, which already rejects an unknown identifier. |
| Third-party certificate-hash transparency or trusted timestamping | Planned | Publish certificate hashes to a third-party-controlled transparency log or timestamping authority and retain the returned inclusion or timestamp evidence. This would establish only the existence and timing of a record; it would establish neither that provider calls occurred nor that signed provider-call counters are accurate. |
...
| Provider content refusals conflated with provider call failures | Planned | `provider_call_failures` increments identically for infrastructure failures and provider content-policy refusals. A timeout or server error means no evidence was obtained; a content refusal means the provider made a decision, which is itself an outcome worth recording. Both currently drive a `LIVE_GATING_CHECK` certificate to `INCONCLUSIVE` and fail CI. Run `20260805-075449` recorded 150 prompts, 0 leaks and 0 harmless blocked with 49 of 50 provider calls succeeding, with the single failure being an xAI `permission-denied` content refusal (`SAFETY_CHECK_TYPE_BIO`) on `gs-044`, a prompt the gate had passed. The attempts log preserves the provider error text, and the optional downstream sidecar records error metadata, but no structured counter or certificate field separates provider refusals from absent evidence. Consider a distinct counter or outcome class for provider refusals, so a refusal is recorded as provider behaviour rather than as absent evidence. Retry does not address this for the same input because a content-policy refusal should not be expected to become a successful model response merely by retrying. |
...
| Signed certificate does not assert content evaluation | Planned | The signed certificate payload carries no `content_evaluated` count, policy-load-success field, or systemic-reset indicator, so it cannot distinguish a real evaluation from a total policy-load failure in which no prompt reached the gate. Add a signed field asserting how many rows were actually content-evaluated. |
| Signed certificate omits expected uncovered rows | Planned | Include `expected_uncovered` in the signed certificate payload so an `AUDIT FAILED` result is interpretable rather than guaranteed for any suite with known coverage gaps. |
| Certificate contract does not reject zero content evaluation | Planned | Once the signed content-evaluation field exists, `tools/validate_certificate_contract.py` should reject a certificate where `prompts_tested > 0` and the content-evaluated count is zero. The validator currently has no rule that would catch a systemic-reset-derived result. |
| Archived run retention is insufficient for execution classification | Planned | Retention varies across archived runs: some preserve row ledgers, summaries and attempt logs, some retain only `audit.json`, and some certificates record an empty `commit_sha`. Run `20260416-003926-000000-049b1e770f3d` could not be classified from committed evidence for this reason. Define and enforce a minimum retained evidence set per archived run. |
| Systemic reset counted as prompt-level result | Planned | Any run containing `systemic_reset_domain_pack_load_failed` must be classified `INCONCLUSIVE` or rejected, never scored. Use `canary_fail` as the test case. |
...
Automated characterisation records that normalisation is not idempotent where marker recovery appends an already-present canonical marker.
...
Automated characterisation records that exact normalised-string invariance does not hold where an invisible character is inserted inside a marker token.
