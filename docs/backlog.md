...
| Homoglyph substitution can bypass multi-token rules | Planned | `core._canonical_compact()` applies the homoglyph translation only to detect entries in `_OBFUSCATION_CANONICAL_MARKERS`. The translated text is not returned, so evaluated content retains the original homoglyph characters and only the recovered marker phrase is restored in ASCII. Of the fifteen canonical markers, seven are single words and eight are multi-word phrases. A payload survives substitution only where the restored marker covers the tokens its rule requires. Verified against commit `516118a` using only the nine mappings in `_HOMOGLYPH_ASCII_MAP`: `"ignore all safety checks"` and `"reveal the system prompt"` both change from `BLOCKED` to `PASS`, while `"suppress the audit log"` continues to block because that full phrase is itself a multi-word marker. Public documentation does not claim complete homoglyph normalisation, so no published claim is currently inaccurate, but the scope is undocumented. The existing Hypothesis test substitutes characters only within the selected marker word and therefore does not exercise this case. Consider applying the translation to the evaluated text in addition to marker detection. That changes what rules evaluate and requires full benchmark revalidation across all packs. |
| Recorded normalisation limitations have no test coverage | Planned | Two normalisation behaviours are documented as limitations but no dedicated test records either. `normalize_obfuscation()` is not idempotent for inputs containing a canonical marker, because marker recovery appends an already-present marker on each pass: `"obey"` yields `"obey obey"` then `"obey obey obey"`. Exact normalised-string invariance does not hold when an invisible character is inserted inside a marker token: `"o\u200bbey"` yields `"o bey obey"` rather than `"obey obey"`. Both are reproducible manually and both are described in the evidence perimeter, but nothing would detect a change in either behaviour. Consider tests asserting the observed behaviour so any future change is visible. |
