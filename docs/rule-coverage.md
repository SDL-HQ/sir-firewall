...
read-only analysis tool. The tool evaluates suite content in memory and, by
default, writes only the two output paths explicitly supplied by the operator. It does not
