...
**Latest-run marker correction:** The single-run workflow previously derived the unsigned `docs/latest-run.json` status from the certificate output path rather than reading the signed certificate's `result`. Because both `AUDIT FAILED` and `INCONCLUSIVE` certificates use the non-canonical local output path, conclusive `AUDIT FAILED` runs could be recorded as `INCONCLUSIVE` in the unsigned latest-run marker. The marker now maps `PASS`, `FAIL`, and `INCONCLUSIVE` from the signed certificate result and reports missing sidecar artefacts separately as diagnostics. Signed certificates were correct throughout; no published certificate was affected. The `evidence_artifacts_complete` and `missing_evidence_artifacts` diagnostic fields are emitted by the single-run marker; the benchmark marker, which rewrites the same file from the run index, does not currently emit them, so consumers should treat them as optional.
